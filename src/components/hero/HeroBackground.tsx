  ...
  useEffect(() => {
    ...
    function buildParticles() {
      ...
      for (let ci = 0; ci < cols; ci++) {
        ...
        for (let j = 0; j < CHARS_PER_COL; j++) {
          const y =
            (j / CHARS_PER_COL) * c.height +
            Math.random() * (c.height / CHARS_PER_COL);
    ...
    function draw() {
      ...
      for (const p of particles.current) {
        if (ms.active) {
          ...
          if (dist < FORCE_RADIUS && dist > 1) {
            ...
            const motion = Math.min(
              1,
              Math.sqrt(ms.vx * ms.vx + ms.vy * ms.vy) / 24,
            );
        ...
        // Use global CSS accent color from --accent-rgb (defined in the global CSS)
        const accentRGB =
        ...
        if (excited && speed > 5) {
          ctx.fillStyle = `rgba(${accentRGB}, ${Math.min(1, alpha)})`;
        } else if (isEmote) {
          ctx.fillStyle = `rgba(${accentRGB}, ${Math.min(0.5, alpha * 1.4)})`;
        } else {
  ...
  return (
    <div
      ...
      style={{
        ...
        marginTop: "-180px",
        background:
          "radial-gradient(circle, rgba(var(--accent-rgb), 0.055) 0%, rgba(var(--accent-rgb), 0.01) 50%, transparent 70%)",
        borderRadius: "50%",
        pointerEvents: "none",
        opacity: 0,
